...
def test_whitelist_exit_when_age_gt_6h():
    svc = MonitorService()
...
def test_whitelist_not_exit_when_age_le_6h():
    svc = MonitorService()
    ...
    token.added_at = datetime.now(timezone.utc) - timedelta(hours=5)
    token.fdv = 50000
    svc.tokens[token.address] = token
...
def test_seconds_until_next_tick_aligns_to_1m_boundary_with_buffer():
    now_ts = 12 * 3600 + 3 * 60 + 20
    assert seconds_until_next_tick(now_ts=now_ts, interval_seconds=60, buffer_seconds=5) == 45


def test_seconds_until_next_tick_negative_buffer_is_clamped():
    now_ts = 12 * 3600 + 4 * 60 + 59
    assert seconds_until_next_tick(now_ts=now_ts, interval_seconds=60, buffer_seconds=-7) == 1
