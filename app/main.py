...
class MonitorService:
    ...
    async def _handle_whitelist_exit(self, token: TokenRecord) -> None:
        ...
        too_old_and_low_fdv = token.fdv is not None and token.fdv < 50000 and age_hours > 24
        if not (too_low_fdv or too_old_and_low_fdv):
            return
        ...
        self.logs.append(
            SignalLog(
                ...
                address=token.address,
                strategy=StrategyName.SELL_ONLY,
                signal=Signal.SELL if (too_low_fdv or too_old_and_low_fdv) else Signal.HOLD,
                reason="白名单移除并加入黑名单",
            )
        )
        self.save_state()
